...
export default function (api, req, opts, metadata, dbOpts, idbChanges, callback) {
  ...
  function fetchExistingDocs(txn, docs) {
    ...
    docs.forEach(function (doc) {
      if (doc.id.startsWith('_local/')) {
        txn.objectStore(META_STORE).get(doc.id).onsuccess = readDone;
      } else {
        txn.objectStore(DOC_STORE).index('_id').get(doc.id).onsuccess = readDone;
      }
    });
  }
  ...
  function write(txn, doc, i) {
    ...
    const mustDeleteOldDoc = !doc.isNewDoc && doc.wasDeleted !== doc.deleted;
    const wasDeleted = doc.wasDeleted;
    console.log('-----------------------');
    console.log('opts:', opts);
    console.log('req:', req);
    console.log('doc:', doc);
    console.log('doc.id:', doc.id);
    console.log('isNewDoc:', doc.isNewDoc);
    console.log('wasDeleted:', wasDeleted);
    console.log('nowDeleted:', doc.deleted);
    console.log('mustDeleteOldDoc:', mustDeleteOldDoc);
    delete doc.isNewDoc;
    ...
    // FIXME for now, we could just delete any previous doc?  This should at least
    // tell us if we're going in the right direction
    const deleteDeleted = txn.objectStore(DOC_STORE).delete([ 1, doc.id ]);
    deleteDeleted.onsuccess = deleteDeleted.onerror = function () {
      const deleteNotDeleted = txn.objectStore(DOC_STORE).delete([ 0, doc.id ]);
      deleteNotDeleted.onsuccess = deleteNotDeleted.onerror = doPut;
    };

    // The code below looks correct but seems to fail when the same doc is rewritten multiple times
    // in the same call to bulkDocs with differing values for deleted.  There's a failing test for
    // this in the standard suite.

//    if (!isLocal && mustDeleteOldDoc) {
//      // Because the primary index of the DOC_STORE is [deleted, id], if deleted changes,
//      // we cannot directly overwrite a doc by putting it.  Putting will then fail because
//      // the ID index will have its uniqueness constraint violated.  Instead, we must
//      // manually delete the old version, which will remove it from the index, and then
//      // re-add it with its new values immediately afterwards.
//      txn.objectStore(DOC_STORE).delete([ wasDeleted ? 1 : 0, doc.id ]).onsuccess = doPut;
//    } else {
//      doPut();
//    }

    function doPut() {
      const putRequest = txn.objectStore(docStore).put(doc);
      putRequest.onerror = function (err) {
        console.log('bulkDocs().put() failed:', err);
        debugger;
      };
  ...
  // TODO merge duplicate docs somehow...
