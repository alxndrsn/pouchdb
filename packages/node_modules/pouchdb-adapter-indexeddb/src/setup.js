...
function openDatabase(openDatabases, api, opts, resolve, reject) {
  console.log('indexeddb.openDatabase()', opts);
  ...
  openReq.onupgradeneeded = function (e) {
    ...
    console.log('indexeddb.onupgradeneeded', 'oldVersion:', e.oldVersion, '; old pouchdbVersion:', pouchdbVersion);

    if (pouchdbVersion < 2) {
      // deliberately corrupt local objects
      ...
      allDocsReq.onsuccess = event => {
        ...
        doc.data.migrated = true;
        Object.values(doc.revs).forEach(rev => {
          rev.data.migrated = true;
        });

        cursor.update(doc).onsuccess = () => {
          cursor.continue();
        };
      };
    }
