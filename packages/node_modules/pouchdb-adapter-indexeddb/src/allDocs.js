...
export default function (txn, metadata, opts, callback) {
  console.log('allDocs()', 'ENTRY', JSON.stringify(opts));
  if (txn.error) {
  ...
  function fetchResults() {
    ...
    function fetchNextBatch() {
      console.log('fetchNextBatch().getAll()', JSON.stringify({ keyRange:forPrint(keyRange), batchSize }));
      dbIndex.getAll(keyRange, batchSize).onsuccess = (e) => {
        const batch = e.target.result;
        console.log('fetchNextBatch()', 'got batch with length:', batch.length);
        console.log('fetchNextBatch()', 'batch:', JSON.stringify(batch));
        ...
        if (batch.length >= batchSize) {
          ...
          const limitKey = descending ? lastSeenKey : keyRange.upper;
          if (startKey[1] !== limitKey[1]) {
            const _inclusiveEnd = descending ? false : inclusiveEnd;
            const _inclusiveStart = descending ? true : false;
            console.log('creating new key range:', JSON.stringify({descending:!!descending, oldKeyRange:forPrint(keyRange), startKey, limitKey}));
            keyRange = createKeyRange(startKey, limitKey, _inclusiveEnd, key, descending, _inclusiveStart);
            console.log('created new key range:', JSON.stringify(forPrint(keyRange)));
            return fetchNextBatch();
          }
        } 
        if (descending) results.reverse();
