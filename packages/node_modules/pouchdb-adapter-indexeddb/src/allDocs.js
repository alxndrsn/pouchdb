...
const log = (...args) => console.log('[indexeddb]', Date.now(), ...args);

const LOCAL_PREFIX = '_local';

function allDocsKeys(keys, docStore, allDocsInner) {
	log('allDocsKeys()', { keys, docStore });
...
export default function (txn, metadata, opts, callback) {
  ...
  var descending = 'descending' in opts && opts.descending ? 'prev' : null;
	...
	log('Initialised opts:', { start, end, key, keys, skip, limit, inclusiveEnd, descending });

  var keyRange;
  if (!keys) {
		log('start', 'generate key range');
    keyRange = createKeyRange(start, end, inclusiveEnd, key, descending);
		log('end', 'generate key range');
    if (keyRange && keyRange.error) {
      return handleKeyRangeError(opts, metadata, keyRange.error, callback);
    }
  }

	log('start', 'get docStore');
  var docStore = txn.txn.objectStore(DOC_STORE);
	log('end', 'get docStore');

  txn.txn.oncomplete = onTxnComplete;

  if (keys) {
		log('keys provided; returning allDocsKeys()');
  ...
  function allDocsInner(doc) {
    ...
    var row = {
	  ...
	  log('allDocsInner()', doc, { skip, limit });
  ...
  cursor.onsuccess = function (e) {
		log('cursor.onsuccess', e, 'stack-depth:', new Error().stack.split('\n').length);
