...
function tryCatchInChangeListener(self, change, pending, lastSeq) {
  console.log('tryCatchInChangeListener()', { change, pending, lastSeq });
  // isolate try/catches to avoid V8 deoptimizations
  try {
    const ll = self.listeners('change');
    console.log('change listener count:', ll.length);
    console.log('change listeners:', ll.map(f => f.toString()));
    self.emit('change', change, pending, lastSeq);
  } catch (e) {
    console.log('Caught error when emitting change!', e);
    guardedConsole('error', 'Error in .on("change", function):', e);
  }
}

function processChange(doc, metadata, opts) {
  console.log('processChange() :: ENTRY');
