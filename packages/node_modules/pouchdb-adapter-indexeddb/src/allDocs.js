...
export default function (txn, metadata, opts, callback) {
  ...
  if (skip || limit) {
    ...
    console.log('allDocs()', JSON.stringify({ skip, limit, limitKey, inclusive_end:opts.inclusive_end, descending }));
    console.log('allDocs()', JSON.stringify({ limitKey, lower:keyRange.lower, upper:keyRange.upper }));

    dbIndex.openKeyCursor(keyRange, descending || 'next').onsuccess = (e) => {
      const cursor = e.target.result;
      //if (!cursor) {
      //  console.log('allDocs()', 'empty cursor - committing and returning...');
      ...
      if (firstKey === undefined) {
        if (!cursor) return txn.txn.commit();
        firstKey = cursor.key;
        console.log('Got firstKey:', JSON.stringify(firstKey));
        if (firstKey === undefined) {
          // no results
          return txn.txn.commit();
        }
      ...
      if (limit) {
        ...
        if (limit > 1 && cursor) {
          console.log('limit > 1; advancing by:', limit - 1);
          // set up keyRange in case we go over the end...
      ...
      if (limitKey && cursor) {
        console.log('limitKey requested...');
        ...
        console.log('No limitKey requested, or cursor empty...');
        limitKey = descending ? keyRange.lower : keyRange.upper;
      }
      console.log('allDocs()', JSON.stringify({ limitKey, lower:keyRange.lower, upper:keyRange.upper }));
      ...
      if (keyRange.error) {
        console.log('key range error:', keyRange.error);
        txn.txn.abort();
        return handleKeyRangeError(opts, metadata, keyRange.error, callback);
      }
