...
function HttpPouch(opts, callback) {
  ...
  const ourFetch = async function (url, options) {
	  const iterationNumber = window.iterationNumber;
		const log = (...args) => console.log(iterationNumber, 'ourFetch()', ...args);
		log('ENTRY', url, options);
		options = options || {};
		...
		log('calling fetchFun:', fetchFun.toString());
		try {
  ...
  function adapterFun(name, fun) {
	...
	async function _fetchJSON(url, options) {
		while (ongoingFetch) {
			const current = ongoingFetch;
			try {
				await current;
			} catch (_) {
				// somebody else's problem
			}
			if (current === ongoingFetch) {
				ongoingFetch = null;
			}
		}
		return ongoingFetch = _fetchJSON(url, options);
	}

  window.fetchCount = -1;
  async function fetchJSON(url, options) {
		const fetchCount = ++window.fetchCount;
	  const iterationNumber = window.iterationNumber;
		const log = (...args) => !url.includes('longpoll') && console.log(iterationNumber, 'adapter-http.fetchJSON()', ...args);
    log('ENTRY ::', url, options);
	...
	(options.method && options.method.toLowerCase() !== 'delete')) {
    ...
    log(iterationNumber, fetchCount, 'fetchJSON() :: response received OK:', response.ok, response.status);
    log(iterationNumber, fetchCount, 'fetchJSON() :: bodyUsed:', response.bodyUsed);
    ...
    if (true) {
			await new Promise((resolve, reject) => {
				...
				function processNextChunk({ done, value }) {
					try {
						log('processNextChunk()');
						log('processNextChunk()', ++chunkCount, Math.random(), done, typeof value, value, json);

						if (done) {
							log('done!  parsing JSON from:', json);
							try {
						...
						//json += String(value);
						json += value.reduce((acc, c) => acc + String.fromCharCode(c), '');
						log('cumulative JSON:', json);

						reader.read().then(processNextChunk).catch(reject);
					} catch (err) {
						reject(err);
					}
				}
			...
			try {
				...
				log('response.text =', response.text.toString());
				log('awaiting text...');
				const text = await response.text();
				log('fetchJSON() :: response text:', text);
				json = JSON.parse(text);//await response.json();
				log('fetchJSON() response.json() returned:', json);
			} catch(err) {
				log(iterationNumber, fetchCount, 'fetchJSON() response.text/json() threw:', err);
				//log(await response.text());
				throw err;
			}
		}
		log('Read JSON as:', json);
  ...
  api.bulkGet = coreAdapterFun('bulkGet', function (opts, callback) {
    ...
    async function doBulkGet(cb) {
      const invocationNumber = Math.random().toString().split('.')[1];
      const log = (...args) => log('adapter-http.doBulkGet()', invocationNumber, ...args);
  ...
  api._bulkDocs = async function (req, opts, callback) {
		const iterationNumber = window.iterationNumber;
		const log = (...args) => console.log(iterationNumber, 'adapter-http._bulkDocs()', ...args);
  ...
  api._changes = function (opts) {
		const iterationNumber = window.iterationNumber;
		const log = (...args) => console.log(iterationNumber, 'adapter-http._changes()', ...args);
    log(opts);
    ...
    const fetched = function (err, res) {
			const iterationNumber = window.iterationNumber;
			const log = (...args) => console.log(iterationNumber, 'adapter-http.fetched()', ...args);
      log({ err, res, opts });
      if (opts.aborted) {
        return;
      }
  ...
  api._destroy = async function (options, callback) {
    try {
      console.log('Trying to destroy:', host);
      const json = await fetchJSON(genDBUrl(host, ''), {method: 'DELETE'});
      console.log('Destroyed:', host);
      callback(null, json);
    } catch (error) {
      if (error.status === 404) {
        callback(null, {ok: true});
