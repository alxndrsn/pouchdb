...
function replicate(src, target, opts, returnValue, result) {
  ...
  function getBatchDocs() {
    ...
    console.log('getBatchDocs()', invocationNumber, currentBatch.diffs, returnValue);
    return getDocs(src, target, currentBatch.diffs, returnValue).then(function (got) {
      console.log('getBatchDocs()', invocationNumber, 'got:', got);
  ...
  function processPendingBatch(immediate) {
    console.log('processPendingBatch()', 'pendingBatch.changes:', pendingBatch.changes);
    if (pendingBatch.changes.length === 0) {
      if (batches.length === 0 && !currentBatch) {
        ...
        if (changesCompleted) {
          console.log('processPendingBatch()', 'changesCompleted::', changesCompleted);
          completeReplication();
        }
      }
      return;
  ...
  function onChange(change, pending, lastSeq) { // helo
    /* istanbul ignore if */
    if (returnValue.cancelled) {
      console.log('onChange()', 'returnValue.cancelled');
    ...
    console.log('onChange()', 'filterChange() ->', filter);
    if (!filter) {
    ...
    console.log('onChange()', 'emitting ceckpoint...');
    returnValue.emit('checkpoint', { 'pending_batch': pendingBatch.seq });
    nextTick(function () {
      try {
        console.log('onChange()', 'calling processPendingBatch()');
        processPendingBatch(batches.length === 0 && changesOpts.live);
      } catch (err) {
        console.log('onChange()', 'processPendingBatch() threw:', err);
      }
    });
  }
