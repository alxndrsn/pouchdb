...
import { MAIN_DOC_STORE, META_STORE, idbError } from './util';
...
export default function (api, req, opts, metadata, dbOpts, idbChanges, callback) {
  ...
  function fetchExistingDocs(txn, docs) {
    ...
    for(let i=docs.length-1; i>=0; --i) {
      const id = docs[i].id;
      const docStore = id.startsWith('_local/') ? META_STORE : MAIN_DOC_STORE;
      txn.objectStore(docStore).get(id).onsuccess = readDone;
    }
  }
  ...
  function write(txn, doc, i) {
    ...
    for(let i=revsToDelete.length-1; i>=0; --i) {
    ...
    const docStore = isLocal ? META_STORE : MAIN_DOC_STORE;
  ...
  preProcessAttachments().then(function () {
    // We _could_ check doc ids here, and only open the META_STORE if
    // all docs are _local.  This will slow things down for non-local
    // docs, as they always cause updates to the META_STORE.
    api._openTransactionSafely([MAIN_DOC_STORE, META_STORE], 'readwrite', function (err, _txn) {
      if (err) {
        return callback(err);
      }
