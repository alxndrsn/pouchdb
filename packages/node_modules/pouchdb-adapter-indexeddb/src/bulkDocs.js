...
export default function (api, req, opts, metadata, dbOpts, idbChanges, callback) {
  ...
  function fetchExistingDocs(txn, docs) {
    ...
    function readDone(e) {
      ...
      if (++fetched === docs.length) {
        processDocs(txn, docs, oldDocs, revsLimit);
      }
    }
  ...
  function processDocs(txn, docs, oldDocs, revsLimit) {
    ...
    docs.forEach(function (doc, i) {
      ...
      // Update the existing document
        ...
        newDoc = update(txn, doc, oldDocs[doc.id], revsLimit);
      ...
      // New document
        ...
        var merged = merge([], doc.rev_tree[0], isLocalId(doc.id) ? 1 : revsLimit);
  ...
  function update(txn, doc, oldDoc, revsLimit) {
    ...
    var merged = merge(oldDoc.rev_tree, doc.rev_tree[0], isLocalId(doc.id) ? 1 : revsLimit);
    doc.stemmedRevs = merged.stemmedRevs;
    doc.rev_tree = merged.tree;
