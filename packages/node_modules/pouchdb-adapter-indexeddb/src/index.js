      ...
      const finishOpenRevs = () => {
        ...
        for (let i=leaves.length-1; i>=0; --i) {
          ...
          if (!(rev in mySpecialDoc.revs)) {
            ...
            const doc = mySpecialDoc.revs[rev].data;
            doc._id = mySpecialDoc.id;
            doc._rev = rev;
            ...
            if (opts.revs) {
              processRevsRevsInfo(id, doc, mySpecialDoc.rev_tree, opts);
            }

            processing.push(new Promise((resolve, reject) => {
              const cb = (err, res) => err ? reject(err) : resolve(res);
              processAttachments(api, mySpecialDoc, doc, opts, NO_CTX_REQUIRED, cb);
            }));

            myResult.push({ ok: doc });
          }
        }
        cb(null, myResult);
      };
