...
export default function (api, req, opts, metadata, dbOpts, idbChanges, callback) {
  ...
  function fetchExistingDocs(txn, docs) {
    ...
    docs.forEach(function (doc) { // TODO replace with normal index-based iteration
  ...
  function processDocs(txn, docs, oldDocs) {
    ...
    docs.forEach(function (doc, i) { // TODO replace with normal index-based iteration
      var newDoc;

      // The first document write cannot be a deletion
      if ('was_delete' in opts && !(Object.prototype.hasOwnProperty.call(oldDocs, doc.id))) { // TODO just use .wasdelete??  or could opts be null or something
  ...
  function write(txn, doc, i) {
    ...
    if (autoCompaction && !isNewDoc) {
      revsToDelete.concat(compactTree(doc));
    }

    for (let i=revsToDelete.length; i; ) {
    ...
    if (theDoc._attachments) {
      for (var k in theDoc._attachments) { // TODO replace with normal index-based iteration
  ...
  function preProcessAttachments() {
    var promises = docs.map(function (doc) {
      ...
      return Promise.all(attachments).then(function (newAttachments) {
        var processed = {};
        newAttachments.forEach(function (attachment) { // TODO replace with normal index-based iteration
