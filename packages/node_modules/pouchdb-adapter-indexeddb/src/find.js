...
function getIndexHandle(pdb, fields, reject) {
  ...
  return new Promise(function (resolve) {
    pdb._openTransactionSafely([MAIN_DOC_STORE], 'readonly', function (err, txn) {
      if (err) {
      ...
      var existingIndexNames = Array.from(txn.objectStore(MAIN_DOC_STORE).indexNames);
      ...
      if (existingIndexNames.indexOf(indexName) === -1) {
        ...
        resolve(txn.objectStore(MAIN_DOC_STORE).index(indexName));
      }
    });
  });
}
