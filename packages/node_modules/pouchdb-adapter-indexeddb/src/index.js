...
import { META_STORE, MAIN_DOC_STORE } from './util';
...
function IdbPouch(dbOpts, callback) {
  ...
  api._get = $t(get, [META_STORE, MAIN_DOC_STORE]); // TODO can be more selective with which store we're opening here
  ...
  api._getAttachment = $t(getAttachment, [META_STORE, MAIN_DOC_STORE]); // TODO can _local docs have attachments?  Should we decide _which_ store to open here?

  api._changes = $t(function (txn, opts) {
    changes(txn, idbChanges, api, dbOpts, opts);
  }, [MAIN_DOC_STORE]);

  api._getRevisionTree = $t(getRevisionTree, [META_STORE, MAIN_DOC_STORE]); // TODO as per get(), we could check and only open correct store here
  api._doCompaction = $t(doCompaction, [MAIN_DOC_STORE], 'readwrite');
  ...
  api._purge = $t(purge, [MAIN_DOC_STORE], 'readwrite');
