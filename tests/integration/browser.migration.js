...
describe('migration', function () {
  ...
  function usingIdb() {
    ...
    console.log('usingIdb()', 'pref:', pref);
  ...
  function usingIndexeddb() {
    console.log('usingIndexeddb()', 'stack:', new Error().stack);
    const pref = PouchDB.preferredAdapters;
    console.log('usingIndexeddb()', 'pref:', pref);
  ...
  function usingDefaultPreferredAdapters() {
    const pref = PouchDB.preferredAdapters;
    console.log('usingDefaultPreferredAdapters()', 'pref:', pref);
  ...
  before(function () {
    ...
    return Promise.all(scenarios.map(function (scenario) {
      const v = versionNumberFrom(scenario);

      if (!v) {
        return testUtils.Promise.resolve();
      }

      const loader = loadScript('deps/pouchdb-' + v + '-postfixed.js');

      if (usingIndexeddb() && isPost721(scenario)) {
        // REVIEW can we load them in parallel?  Alternative:
        // return testUtils.Promise.all([ loader, loadScript(...) ]);
        return loader.then(() => loadScript('deps/pouchdb-' + v + '-indexeddb-postfixed.js'));
      } else {
        return loader;
      }
    }));
  ...
  scenarios.forEach(function (scenario) {
    ...
    describe('migrate from ' + scenario, function () {
      ...
      describe('indexeddb', () => {
        if (isPost721(scenario)) {
          it('Should support indexeddb->indexeddb upgrades', () => {
            if (skip || !usingIndexeddb()) { return; }

            const log = (...args) => console.log('indexeddb->indexeddb', ...args);
...
function isPost721(scenario) {
  const v = versionNumberFrom(scenario);
  if (!v) {
    return false;
  }

  return versionGte(v, '7.2.1');
}
...
function versionNumberFrom(scenario) {
