  ...
  _setup() {
    ...
    this.revsDiff = adapterFun('revsDiff', function (req, opts, callback) {
      ...
      function processDoc(id, rev_tree) {
        ...
        for (let i=missingForId.length-1; i>=0; --i) {
          addToMissing(id, missingForId[i]);
        }
      }
      ...
      for (let i=0; i<ids.length; ++i) {
        ...
        this._getRevisionTree(id, function (err, rev_tree) {
          ...
          if (++count === ids.length) {
            ...
            // Could just use Object.fromEntries(); is it supported well-enough?
            var missingObj = {};
    ...
    this.get = adapterFun('get', function (id, opts, cb) {
      ...
      if (opts.open_revs) {
        if (opts.open_revs === "all") {
          this._getRevisionTree(id, function (err, rev_tree) {
            ...
            leaves = collectLeaves(rev_tree).map(getRev);
            finishOpenRevs();
          });
        } else {
      ...
      return this._get(id, opts, (err, result) => {
        ...
        if (opts.revs || opts.revs_info) {
          ...
          const lookingFor = doc._rev.split('-')[1];
          let indexOfRev = path.ids.length;
          while (path.ids[--indexOfRev] !== lookingFor) { /* repeat */ }

          // discard after indexOfRev, and reverse path.ids into _revisions and _revs_info

          if (opts.revs) {
            const ids = new Array(indexOfRev);
            for (let i=indexOfRev-1; i>=0; --i) {
              const j = indexOfRev - i - 1;
              ids[j] = path.ids[i].id;
            }
            doc._revisions = {
          ...
          if (opts.revs_info) {
            const revs_info = new Array(indexOfRev);
            for (let i=indexOfRev-1; i>=0; --i) {
              const rev = path.ids[i];
              const j = indexOfRev - i - 1;
              revs_info[j] = {
                rev: (path.pos + i) + '-' + rev.id,
    ...
    this.bulkDocs = adapterFun('bulkDocs', function (req, opts, callback) {
      ...
      // create a copy here
      const ids = [];
      for (let i=req.docs.length-1; i>=0; --i) {
        ids[i] = req.docs[i]._id;
      }
    ...
    this.destroy = adapterFun('destroy', function (opts, callback) {
      ...
      this.get('_local/_pouch_dependentDbs', (err, localDoc) => {
        ...
        const names = Object.keys(dependentDbs);
        const deletedMap = new Array(names.length);
        for (let i=0; i<names.length; ++i) {
          ...
          deletedMap[i] = new PouchDB(trueName, this.__opts).destroy();
        }
...
function getRev(x) { return x.rev; }
