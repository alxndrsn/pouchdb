...
function createAbstractMapReduce(localDocName, mapper, reducer, ddocValidator) {
  ...
  async function httpQuery(db, fun, opts) {
    ...
    if (typeof fun === 'string') {
      ...
      const path = '_design/' + parts[0] + '/_view/' + parts[1] + params;

      const response = await db.fetch(path, {
      ...
      if (!ok) {
        ...
        throw generateErrorFromRequestAndResponse(method, path, result);
      }
    ...
    if (!ok) {
      result.status = response.status;
      throw generateErrorFromRequestAndResponse('POST', path, result);
    }
...
function generateErrorFromRequestAndResponse(method, path, result) {
  const err = generateErrorFromResponse(result);
  err.message = `[${method} ${path}] ${err.message}`;
  return err;
}
