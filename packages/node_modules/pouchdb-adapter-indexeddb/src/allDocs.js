    ...
    function fetchNextBatch() {
      dbIndex.getAll(keyRange, batchSize).onsuccess = (e) => {
        ...
        if (batch.length >= batchSize) {
          const startKey = [0, batch[batch.length-1].id ];
          const limitKey = descending ? keyRange.lower : keyRange.upper;
          console.log('creating new key range:', JSON.stringify({startKey, limitKey}));
          keyRange = createKeyRange(startKey, limitKey, inclusiveEnd, key, descending, false);
          console.log('created new key range:', JSON.stringify(keyRange));
          fetchNextBatch();
        } else {
