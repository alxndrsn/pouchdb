    ...
    this.get = adapterFun('get', function (id, opts, cb) {
      ...
      return this._get(id, opts, (err, result) => {
        ...
        if (opts.revs || opts.revs_info) {
          ...
          var indexOfRev = path.ids.findIndex(x => x.id === lookingFor) + 1;
          var howMany = path.ids.length - indexOfRev;
          path.ids.splice(indexOfRev, howMany); // TODO do we need this?
          path.ids.reverse(); // TODO do we need this?

          if (opts.revs) {
            doc._revisions = {
              start: (path.pos + path.ids.length) - 1,
              ids: path.ids.map(getId),
            };
          }
          if (opts.revs_info) {
            const revs_info = new Array(path.ids.length);
            for (let i=path.ids.length-1; i>=0; --i) {
...
function get_id(x) { return x._id; }
