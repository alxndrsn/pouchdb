...
function init(api, opts, callback) {
  ...
  function createSchema(db, dbName) {
    ...
    // added in v4
    if (!dbName.includes('-mrview')) { // conditional added in v6
  ...
  function migrateAttsAndSeqsAgain(txn, callback, dbName) {
    if (dbName.includes('-mrview-')) {
      var attAndSeqStore = txn.objectStore(ATTACH_AND_SEQ_STORE);
      attAndSeqStore.deleteIndex('seq');
      attAndSeqStore.deleteIndex('digestSeq');
    }
    callback();
  }
  ...
  req.onupgradeneeded = function (e) {
    ...
    if (e.oldVersion < 1) {
      return createSchema(db, dbName); // new db, initial schema
    }
    // do migrations
