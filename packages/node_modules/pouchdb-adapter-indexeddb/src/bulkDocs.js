...
import { LOCAL_DOC_STORE, MAIN_DOC_STORE, META_STORE, idbError } from './util';
...
export default function (api, req, opts, metadata, dbOpts, idbChanges, callback) {
  ...
  function fetchExistingDocs(txn, docs) {
    ...
    docs.forEach(function (doc) {
      ...
      const docStore = isLocal ? LOCAL_DOC_STORE : MAIN_DOC_STORE;
  ...
  function write(txn, doc, i) {
    ...
    if (isLocal && doc.deleted) {
      txn.objectStore(LOCAL_DOC_STORE).delete(doc.id).onsuccess = function () {
        results[i] = {
          ok: true,
          id: doc.id,
          rev: '0-0'
  ...
  preProcessAttachments().then(function () {
    // TODO could check doc ids here, and only open LOCAL and/or MAIN store as required
    api._openTransactionSafely([LOCAL_DOC_STORE, MAIN_DOC_STORE, META_STORE], 'readwrite', function (err, _txn) {
      if (err) {
        return callback(err);
      }
