...
function allDocsKeys(keys, docStore, allDocsInner) {
  ...
  let count = 0;
  keys.forEach(function (key, index) { // TODO replace with normal index-based iteration
    docStore.get(key).onsuccess = function (event) {
      valuesBatch[index] = event.target.result || {key: key, error: 'not_found'};
      if (++count === keys.length) {
        valuesBatch.forEach(allDocsInner); // TODO replace with normal index-based iteration
      }
    };
  });
}
...
export default function (txn, metadata, opts, callback) {
  ...
  function include_doc(row, doc) {
    ...
    if (opts.attachments && docData._attachments) {
      for (var name in docData._attachments) { // TODO replace with normal index-based iteration
