...
export default function (api, req, opts, metadata, dbOpts, idbChanges, callback) {
  ...
  function docsRevsLimit(doc) {
    return doc.id.startsWith(LOCAL_PREFIX) ? 1 : revsLimit;
  }
  ...
  function update(txn, doc, oldDoc) {
    ...
    const isRoot = doc.rev.startsWith(ROOT_PREFIX);
  ...
  function write(txn, doc, i) {
    ...
    const isLocal = doc.id.startsWith(LOCAL_PREFIX);
  ...
  // FIXME looks like this can be done externally - will just want to pass in preProcessAttachment() function
