...
import { winningRev as calculateWinningRev, merge, compactTree, isLocalId } from 'pouchdb-merge';
...
export default function (api, req, opts, metadata, dbOpts, idbChanges, callback) {
  ...
  function fetchExistingDocs(txn, docs) {
    ...
    function readDone(e) {
      ...
      if (++fetched === docs.length) {
        processDocs(dbOpts, txn, docs, oldDocs);
      }
    }

    for (let i=0; i<docs.length; ++i) {
      const docId = docs[i].id;
      const docStore = isLocalId(docId) ? META_STORE : DOC_STORE;
      txn.objectStore(docStore).get(docId).onsuccess = readDone;
    }
  }

  function processDocs(dbOpts, txn, docs, oldDocs) {
    ...
    docs.forEach(function (doc, i) {
      ...
      // Update the existing document
        ...
        newDoc = update(dbOpts, opts, txn, doc, oldDocs[doc.id]);
      ...
      // New document
        ...
        var merged = merge([], doc.rev_tree[0], isLocalId(doc.id) ? 1 : dbOpts.revs_limit);
  ...
  function write(txn, doc, i) {
    ...
    // delete their data
    if (doc.stemmedRevs) {
      for (let i=0; i<doc.stemmedRevs.length; ++i) {
        delete doc.revs[doc.stemmedRevs[i]];
      }
    ...
    if (autoCompaction && !isNewDoc) {
      ...
      for (let i=0; i<result.length; ++i) {
        delete doc.revs[result[i]];
      }
    }

    write_processAttachments(txn, doc, theDoc, writtenRev, setError);
  ...
  for (var i=0; i<req.docs.length; i++) {
    try {
      ...
      docs.push(convertDocFormat(parseDoc(req.docs[i], opts.new_edits, dbOpts)));
    } catch (err) {
      return callback(err);
    }
  }

  preProcessAttachments(docs).then(function () {
...
function preProcessAttachments(docs) {
...
function update(dbOpts, opts, txn, doc, oldDoc) {
  ...
  var merged = merge(oldDoc.rev_tree, doc.rev_tree[0], isLocalId(doc.id) ? 1 : dbOpts.revs_limit);
  doc.stemmedRevs = merged.stemmedRevs;
  doc.rev_tree = merged.tree;
...
function write_processAttachments(txn, doc, theDoc, writtenRev, setError) {
    ...
    for (var k in theDoc._attachments) {
      ...
      if (attachment.stub) {
        if (!(attachment.digest in doc.attachments)) {
          setError(createError(MISSING_STUB));
