  ...
  describe('test.all_docs.js-' + adapter, function () {
    ...
    it('Testing allDocs opts.keys', function () {
      ...
      return db.bulkDocs(origDocs).then(function () {
        ...
        result.rows.map(keyFunc).should.deep.equal(keys);
        result.rows[keys.indexOf('2')].value.deleted.should.equal(true, 'deleted doc with keys option: .deleted');
        (result.rows[keys.indexOf('2')].doc === null).should.equal(true, 'deleted doc with keys option: .doc');
      });
    });
